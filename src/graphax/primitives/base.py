...
def make_parallel_jacobian(i, primals, val_out, elemental):
    ...
    if len(primals) == 2 and get_shape(primal) != get_shape(val_out):
        ...
        for j, (os, ps) in enumerate(zip(val_out.aval.shape, primal.aval.shape)):
            ...
            if ps != os:
                axis = None if elemental_is_scalar else sum(1 for d in out_dims if d.axis is not None)
                out_dims.append(DenseIndex(j, os, axis))
                primal_dims.append(
            ...
            for k, d in enumerate(primal_dims[:-1]):
                ...
                primal_dims[k] = replace(d, id=d.id + 1)
...
def standard_elemental_only(elementalrule, primal_out, primals, **params):
    ...
    Returns elementals position-aligned with `primals` (which corresponds to
    `eqn.invars`). _build_graph filters out Literal positions when wiring edges.
    """
