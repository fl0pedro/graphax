...
def _broadcast_elementals(primals, val_out, **params):
    ...
    new_primal_dims = [
        SparseIndex(l + j, primal_shape[j], None, dims[j]) for j in range(n)
    ]
    ...
    def squeeze_transform(pre):
        ...
        for d in new_out_dims:
            ...
            if d.axis is not None:
                d.axis = new_val_axes.index(d.axis)
            if isinstance(d, SparseIndex):
    ...
    def concatenate_transform(primal_idx, pre):
        ...
        if isinstance(d, DenseIndex):
            if d.axis is not None:
                ...
                new_out_dims[dim].axis = new_axis
    ...
    def inverse_concatenate_transform(primal_idx, post):
        ...
        if d is None:
            ...
            if d.axis is not None:
                ...
                def _shift(idx_dim):
                    if idx_dim.axis is None:
                        return
                    if idx_dim.axis >= out_axis:
                        idx_dim.axis += 1
                    if idx_dim.axis >= primal_axis:
                        idx_dim.axis += 1
