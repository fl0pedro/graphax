...
``iota`` / ``device_put`` / ``stop_gradient`` carry no gradient — each returns an
empty elemental list. They route or block gradient flow rather than computing a
new value-dependent Jacobian. (``split`` is a structural slice and lives in
``transforms.py`` alongside the other slice rules.)
"""
