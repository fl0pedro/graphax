...
def _mixed_mul(lhs: SparseTensor, rhs: SparseTensor) -> SparseTensor:
    ...
    for (ld, rd) in zip(lhs.primal_dims, rhs.out_dims):
        if isinstance(ld, SparseDimension) or isinstance(rd, SparseDimension):
            ...
            # Kronecker deltas
            if ld.val_dim is not None and rd.val_dim is not None and lhs.val.shape[ld.val_dim] == ld.size and \
                    rhs.val.shape[rd.val_dim] == rd.size:
                ...
                if isinstance(rd, DenseDimension):
                    ...
                    new_out_dims.insert(ld.other_id,
                                        SparseDimension(ld.other_id, ld.size, lval_dim, rd.other_id - r + l))
                    new_primal_dims.insert(rd.other_id - r,
                                           SparseDimension(rd.other_id - r + l, ld.size, lval_dim, ld.other_id))
            else:
                ...
                if isinstance(rd, DenseDimension):
                    ...
                    val_dim = None
                    if rd.val_dim is not None:
                        val_dim = rd.val_dim - sum(
                            [1 for rc in rcontracting_axes if rc < rd.val_dim]) + lhs.val.ndim - sum(
                            [1 for lc in lcontracting_axes])
                    else:
                    ...
                    if ld.val_dim is not None:
                        ...
                        val_dim = rd.val_dim \
                                  ...
                                  - sum([1 for lb in lbroadcasting_axes])
                    new_out_dims.insert(ld.other_id,
                                        SparseDimension(ld.other_id, ld.size, val_dim, rd.other_id - r + l))
                    new_primal_dims.insert(rd.other_id - r,
                                           SparseDimension(rd.other_id - r + l, ld.size, val_dim, ld.other_id))

    dimension_numbers = (tuple(lcontracting_axes), tuple(rcontracting_axes))
    batch_dimensions = (
    tuple(lbroadcasting_axes), tuple(rbroadcasting_axes))  # we abuse these guys here to handle the SparseDimensions
