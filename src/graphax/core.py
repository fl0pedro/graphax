...
# block-diagonal (GEMM) kernel rather than a full N×N densify. Sound only with the
# idempotent produce_diag re-mask (see produce_diag._KEEP_BLOCKDIAG).
_KEEP_BLOCKDIAG = os.environ.get("GRAPHAX_KEEP_BLOCKDIAG", "0") == "1"


def _is_pure_blockdiag(edge) -> bool:
    """True iff ``edge`` carries at least one meta-block-diagonal (Diagonal) dim
    and NO compressed/implicit dim — i.e. a residual structure the batched
    block-diagonal contraction kernel can consume directly (no densify needed)."""
        ...
        for out_edge in graph[central_var].keys():
            ...
            for in_edge in transpose_graph[central_var].keys():
                ...
                if _is_approx_cfg and _is_approx(edge_outval):
                    # GRAPHAX_KEEP_BLOCKDIAG: when the residual structure is a pure
                    # block-diagonal (Diag) edge — no implicit/Compress dim — keep it
                    # SPARSE so the next contraction routes through the batched
                    # block-diagonal (GEMM) kernel instead of a full N×N densify. The
                    # idempotent re-mask in produce_diag makes a later uniform Diag on
                    # this edge a sound no-op, so the load-bearing densify (which the
                    # under-masking bug otherwise requires) is not needed here.
