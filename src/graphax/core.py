...
def jacve(
    ...
    def jacfun(*args, **kwargs):
        ...
        if count_ops or return_history:
            outputs, aux_data = out
...
def _build_graph(
    ...
    for eqn in jaxpr.eqns:
        ...
        cce = partial(elemental_rules.get(eqn.primitive), **eqn.params)
        primal_outvals, elemental_outvals = cce(invals)
        eqns = jax.make_jaxpr(lambda x: cce(x)[0])(invals)
        ...
        _write_elemental = partial(write_elemental, eqn.outvars[0], eqns=eqns)
...
def vertex_elimination_jaxpr(
    ...
    *args,
    ...
    Sequence[Sequence[jnp.ndarray]], Tuple[Sequence[Sequence[jnp.ndarray]], dict]
]:
    """
    ...
    env, graph, transpose_graph, jaxpr_graph, vo_vertices = _build_graph(
        jaxpr, args, consts
    )
    # print(jaxpr_graph)
    # print(graph, vo_vertices)
    ...
    for i, vertex in enumerate(order):
        ...
        num_mul, num_add = _eliminate_vertex(
            vertex, jaxpr, graph, transpose_graph, iota, vo_vertices
        )
        if count_ops:
    ...
    if sparse_representation:
        jac_vals = [
            graph[invar][outvar] if outvar in list(graph[invar].keys()) else None
            for outvar in jaxpr.outvars
            for invar in jaxpr_invars
        ]
    else:
        jac_vals = [
            graph[invar][outvar].dense(iota)
...
def extract_jaxpr(
    ...
    def eval_graph(*tracer_args):
        ...
        for i in range(len(args)):
            if i in argnums:
                full_args.append(tracer_args[tracer_idx])
                tracer_idx += 1
            else:
