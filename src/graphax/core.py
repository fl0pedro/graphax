...
def jacve(
    ...
    def jacfun(*args, **kwargs):
        ...
        if count_ops:
            ...
            return jtu.tree_unflatten(out_tree, primal_out), jtu.tree_unflatten(
                out_tree, grads
            )
        else:
            out_tree = jtu.tree_structure(tuple(closed_jaxpr.jaxpr.outvars))
...
def _build_graph(
    ...
    for eqn in jaxpr.eqns:
        ...
        if eqn.primitive.name == "split" and eqn.primitive not in elemental_rules:
            from .primitives import defmultielemental, split_elemental_rule

            defmultielemental(eqn.primitive, split_elemental_rule)
        ...
        if eqn.primitive.multiple_results:
            ...
            for i, invar in enumerate(invars):
                if i < len(elemental_outvals):
                    for j, outvar in enumerate(eqn.outvars):
                        if j < len(elemental_outvals[i]):
                            write_elemental(outvar, invar, elemental_outvals[i][j])
        else:
            safe_map(write, eqn.outvars, [primal_outvals])
            invars = [invar for invar in eqn.invars if type(invar) is core.Var]
    ...
    *args,
    ...
    for vertex in order:
        num_mul, num_add = _eliminate_vertex(
            vertex, jaxpr, graph, transpose_graph, vo_vertices
        )
        if count_ops:
    ...
    if count_ops:
        ...
        aux = {"num_muls": num_muls, "num_adds": num_adds, "order_counts": order_counts}
        return jac_vals, aux
    if has_aux:
