...
a massive slowdown. The fix is the static ``_zero_fill`` flag computed at
construction (``_compute_zero_fill_flag``), preserved through ``_copy``, and
read by ``_is_zero_fill`` without re-probing. These tests pin that invariant,
...
def test_zero_fill_matmul_takes_tiled_not_densify():
    ...
    assert a._zero_fill is True and _is_zero_fill(a)
...
def test_zero_fill_propagates_through_matmul_chain():
    a, b = _bd(4, 3, 1), _bd(4, 3, 2)
    r = matmul(a, b)
    assert r._zero_fill is True and _is_zero_fill(r)
    with track_paths() as paths:
        matmul(r, b)
...
def test_compressed_output_stays_fast_path_eligible():
    ...
    # boundary, must keep zero_fill=True so the consumer stays on the fast path.
    ...
    s = au + bu
    assert s._zero_fill is True
    mat = _materialize_for_op(s)
    assert mat._zero_fill is True and _is_zero_fill(mat)
