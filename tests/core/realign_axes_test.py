...
DiagonalIndex always occurs in pairs: one in out_dims, one in primal_dims.
...
_realign_axes is called after DiagonalIndex val_axis is set to None because
...
class RealignAxesTests(unittest.TestCase):
    ...
    def test_squeeze_single_sparse_pair_axis(self):
        """Squeeze DiagonalIndex pair's size-1 axis; remaining DenseIndices shift down."""
        # st.dims = [DiagonalIndex(0), DenseIndex(1), DenseIndex(2), DiagonalIndex(3)]
        ...
        out_dims = (DiagonalIndex(0, 2, 0, 3), DenseIndex(1, 3, 1))
        primal_dims = (DenseIndex(2, 4, 2), DiagonalIndex(3, 2, 0, 0))
        st = SparseTensor(out_dims, primal_dims, jnp.ones((1, 3, 4)))

        # Simulate pre-conditions: DiagonalIndex pair val_axis set to None
    ...
    def test_squeeze_among_two_sparse_pairs(self):
        """Two DiagonalIndex pairs; squeeze one pair's axis, the other pair shifts."""
        # st.dims = [DiagonalIndex(0), DiagonalIndex(1), DenseIndex(2),
        #            DiagonalIndex(3), DenseIndex(4), DiagonalIndex(5)]
        ...
        out_dims = (DiagonalIndex(0, 2, 0, 3), DiagonalIndex(1, 3, 1, 5), DenseIndex(2, 5, 2))
        primal_dims = (DiagonalIndex(3, 2, 0, 0), DenseIndex(4, 4, 3), DiagonalIndex(5, 3, 1, 1))
        st = SparseTensor(out_dims, primal_dims, jnp.ones((1, 3, 5, 4)))

        # Squeeze the first DiagonalIndex pair
        st.out_dims[0].val_axis = None
        ...
        # DiagonalIndex(1): was 1, squeezed axes < 1: [0] => 1 => 0
        self.assertEqual(st.out_dims[1].val_axis, 0)
        ...
        # DiagonalIndex(5): was 1, squeezed axes < 1: [0] => 1 => 0
    ...
    def test_squeeze_axis_above_all_val_axes(self):
        ...
        # st.dims = [DenseIndex(0), DenseIndex(1), DiagonalIndex(2),
        #            DenseIndex(3), DiagonalIndex(4)]
        ...
        out_dims = (DenseIndex(0, 2, 0), DenseIndex(1, 3, 1), DiagonalIndex(2, 4, 3, 4))
        primal_dims = (DenseIndex(3, 5, 2), DiagonalIndex(4, 4, 3, 2))
    ...
    def test_squeeze_axis_between_val_axes(self):
        ...
        # st.dims = [DenseIndex(0), DiagonalIndex(1), DiagonalIndex(2),
        ...
        out_dims = (DenseIndex(0, 2, 0), DiagonalIndex(1, 3, 2, 2))
        primal_dims = (DiagonalIndex(2, 2, 2, 1), DenseIndex(3, 5, 1), DenseIndex(4, 6, 3))
    ...
    def test_squeeze_multiple_axes_mixed_positions(self):
        ...
        # st.dims = [DiagonalIndex(0), DenseIndex(1), DiagonalIndex(2),
        #            DiagonalIndex(3), DiagonalIndex(4), DenseIndex(5)]
        ...
        out_dims = (DiagonalIndex(0, 2, 0, 3), DenseIndex(1, 3, 1), DiagonalIndex(2, 4, 2, 4))
        primal_dims = (DiagonalIndex(3, 2, 0, 0), DiagonalIndex(4, 4, 2, 2), DenseIndex(5, 5, 3))
    ...
    def test_empty_squeeze_list(self):
        ...
        out_dims = (DenseIndex(0, 2, 0), DiagonalIndex(1, 3, 1, 2))
        primal_dims = (DiagonalIndex(2, 2, 1, 1), DenseIndex(3, 4, 2))
    ...
    def test_none_val_axis_unchanged(self):
        ...
        out_dims = (DiagonalIndex(0, 2, None, 3), DenseIndex(1, 3, 0))
        primal_dims = (DenseIndex(2, 4, 1), DiagonalIndex(3, 2, None, 0))
